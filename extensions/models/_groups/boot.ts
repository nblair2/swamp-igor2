/**
 * Boot-stack methods for the `@nblair2/igor2` model: distros, profiles, images
 * and kickstarts.
 *
...
export const methods: Record<string, MethodDef> = {
  distro_create: defineMethod({
    description:
      "Create a boot distro by copying a distro, reusing an image, " +
      "referencing an image ID, or uploading kernel+initrd files",
  ...
  distro_show: defineMethod({
    description: "Fetch a single distro by name and store it",
    arguments: NameArg,
    execute: async (args, context): Promise<MethodResult> => {
      const client = await clientFor(context);
      const res = await client.get("/distros", { query: { name: args.name } });
      const distro = distrosFromData(res.data).find((d) =>
        d.name === args.name
      ) ??
  ...
  distro_edit: defineMethod({
    ...
    execute: async (args, context): Promise<MethodResult> => {
      ...
      if (args.removeGroup !== undefined) fields.removeGroup = args.removeGroup;
      if (args.kickstart !== undefined) fields.kickstart = args.kickstart;
      if (args.public) fields.public = "true";
      ...
      await client.patchForm(`/distros/${encodeURIComponent(args.name)}`, form);
      const finalName = args.newName ?? args.name;
      const res = await client.get("/distros", { query: { name: finalName } });
      const distro = distrosFromData(res.data).find((d) =>
        d.name === finalName
      ) ?? { name: finalName };
  ...
  profile_create: defineMethod({
    description: "Create a boot profile (a distro plus extra kernel arguments)",
  ...
  profile_show: defineMethod({
    description: "Fetch a single profile by name and store it",
    arguments: NameArg,
    execute: async (args, context): Promise<MethodResult> => {
      const client = await clientFor(context);
      const res = await client.get("/profiles", { query: { name: args.name } });
      const profile = profilesFromData(res.data).find((p) =>
        p.name === args.name
  ...
  profile_edit: defineMethod({
    ...
    execute: async (args, context): Promise<MethodResult> => {
      ...
      const finalName = args.newName ?? args.name;
      const res = await client.get("/profiles", { query: { name: finalName } });
      const profile = profilesFromData(res.data).find((p) =>
        p.name === finalName
      ) ?? { name: finalName };
  ...
  image_register: defineMethod({
    description:
      "Register a distro image by uploading kernel+initrd files or " +
      "referencing server-staged files",
  ...
  kickstart_register: defineMethod({
