...
through its REST API: reservations, host power and administration, host
policies, the boot stack (distros, profiles, images, kickstarts), identity
(users, groups, privilege elevation), clusters/MOTD, and server operations.

## What it does

A single model type, `@nblair2/igor2`, exposes the igor2 API as methods.
Connection details and credentials are configured once on the model instance;
every operation is a method on it. The igor2 API is rooted at `/igor` over
HTTPS (default port 8443); authentication uses `GET /igor/login` with HTTP Basic
credentials to obtain a JWT, sent as a `Bearer` token on every request.
...
### Identity, clusters & server operations
...
```bash
# Configure a model instance (credentials are entered once)
swamp model create @nblair2/igor2 igor \
...
# Read the cluster dashboard, list distros, list users
swamp model method run igor show --json
swamp model method run igor distro_list --json
swamp model method run igor user_list --json

# Create a reservation: 5 nodes for 3 days with the "ubuntu20" profile
swamp model method run igor reservation_create \
  --input name=myres \
...
# Power-cycle the reserved nodes, then delete the reservation
swamp model method run igor host_power --input cmd=cycle --input reservation=myres --json
swamp model method run igor reservation_delete --input name=myres --json
```
...
swamp model method run igor distro_create \
...
swamp model method run igor image_register \
...
swamp model method run igor profile_create \
...
```bash
deno fmt extensions/          # format
deno check extensions/**/*.ts # type-check
deno lint extensions/         # lint
deno test extensions/         # run unit tests (stubbed fetch, no server needed)

swamp extension fmt manifest.yaml --check -j
swamp extension quality manifest.yaml --json
```

The model is assembled in `extensions/models/igor.ts` from per-resource modules
under `extensions/models/_groups/` (reservations, hosts, boot, admin), sharing
the HTTP client in `extensions/models/_lib/igor.ts` and the swamp-model plumbing
in `extensions/models/_lib/model.ts`. Unit tests live alongside the source
